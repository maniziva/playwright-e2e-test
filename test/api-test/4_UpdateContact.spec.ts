...
test('Update Contact', async ({ }) => {
  ...
  const UpdateContactResponse = await apiContext.put(baseURL + 'contacts/' + contactId, {
    headers: {
      Authorization: `Bearer ${token}`
    },
    data: contactPayload
  ...
  // Check if the response is successful
  expect(UpdateContactResponse.ok()).toBeTruthy();

  const UpdateContactResponseData = await UpdateContactResponse.json();
  console.log('Update Contact Response:', UpdateContactResponseData);
  await apiContext.dispose();
});
