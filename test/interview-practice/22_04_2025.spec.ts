...
test.describe.parallel('Practice - Day1', async()=>{
    ...
    test('File upload, screenshot and scroll into selector', async({page})=>{
        ...
        await multifileupload.setInputFiles(['./src/data/ContactData.csv','./src/data/ContactData-copy.csv'])
        await page.screenshot({path:'./src/download/fileupload.png',fullPage:true});
    })
    test('static table', async ({page}) =>{
        ...
        for (let i = 1; i < rowCount; i++) {
            ...
            console.log(`Row ${i + 1}: ${BookName?.trim()} | ${Author?.trim()} | ${Price?.trim()}`);
          }
    })
    test('Simple Alert', async({page}) =>{
        const simpleAlert = await page.getByRole('button',{name: 'Simple Alert'});
    ...
    test('Confirmation Alert', async({page}) =>{
        const confirmationAlert = await page.getByRole('button',{name: 'Confirmation Alert'});
    ...
    test.only('Prompt Alert', async({page}) =>{
        const promptAlert = await page.getByRole('button',{name: 'Prompt Alert'}); 
        await page.on('dialog', async(dialog)=>{
            console.log(dialog.message());
